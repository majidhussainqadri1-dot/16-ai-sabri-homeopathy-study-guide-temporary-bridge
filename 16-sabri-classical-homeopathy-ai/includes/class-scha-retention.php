...
final class SCHA_Retention {
    public static function run(): void {
        ...
        $expired = $wpdb->get_results( $wpdb->prepare( "SELECT id,public_id,provider FROM $sessions WHERE retention_until IS NOT NULL AND retention_until <= %s AND status NOT IN ('expired') LIMIT 200", $now ), ARRAY_A ) ?: array();
        foreach ( $expired as $session ) {
            ...
            try {
                ...
                $wpdb->update( $sessions, array( 'status' => 'expired', 'updated_at' => $now ), array( 'id' => $session['id'] ) );
                $wpdb->query( 'COMMIT' );
